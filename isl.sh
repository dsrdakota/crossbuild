...
[[ ! -d isl ]] && git clone git://repo.or.cz/isl.git isl && cd isl || cd isl && git reset --hard && git pull
log "Cleaning stale object files and distribution files for package 'isl' ..."
make clean distclean
log "Generating configuration files for 'isl' ..."
./autogen.sh
log "Configuring for package 'isl' ..."
./configure --build=$HOST_TRIPLET --with-int=gmp --with-gnu-ld=yes --enable-static=yes --enable-shared=no --disable-dependency-tracking --with-sysroot=$CROSS_PREFIX --prefix=$CROSS_PREFIX --with-gmp=system --with-gmp-prefix=$CROSS_PREFIX
[[ $? -eq 0 ]] && log "Building package 'isl' ..." && make -j4 && [[ $? -eq 0 ]] && log "Installing package 'isl' to $CROSS_PREFIX ..." && make install && log "Running tests for package 'isl' ..." && make -j8 check
